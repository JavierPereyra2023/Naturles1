# Add sidebar TOC + scroll-spy to candidate pages — v2 with bug fixes
...
# Color por unidad (basado en path)
...
# Lista de páginas candidatas (37)
$candidates = @(
...
foreach ($rel in $candidates) {
  ...
  # 1) Encontrar las secciones con h2 dentro y construir lista (start, openingTag, title, slug)
  ...
  foreach ($m in $secOpens) {
    $openIdx = $m.Index
    $openTag = $m.Value
    # Encontrar el cierre </section> correspondiente (sin anidamiento, así que el primero después funciona)
  ...
  # 2) Insertar id="slug" en cada apertura de sección (orden inverso)
...
"@
  ...
  # 4) Reemplazar el wrapper del main
  $mainOpenMatch = [regex]::Match($newContent, '<main[^>]*>')
  if (-not $mainOpenMatch.Success) {
    $results += [PSCustomObject]@{ File = $rel; Status = 'NO_MAIN_FOUND'; N = $sections.Count }
    continue
  }
  ...
  if ($idxMainClose -lt 0) {
    $results += [PSCustomObject]@{ File = $rel; Status = 'NO_MAIN_CLOSE'; N = $sections.Count }
    continue
  }
  $newContent = $newContent.Substring(0, $idxMainClose) + '</main></div></div>' + $newContent.Substring($idxMainClose + '</main>'.Length)

  # 5) Insertar CSS .toc-link en el <style> (idempotente)
  $tocCss = @"
  ...
  # 6) Insertar JS scroll-spy antes de </body> (idempotente)
  $scrollJs = @"
