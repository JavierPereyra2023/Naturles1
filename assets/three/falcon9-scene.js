...
const STEPS = ['fairing', 'stage2', 'interstage', 'stage1', 'fins', 'legs'];

// [radius, y] camera waypoints per step, in local rocket units (base = 0, tip ~= 14.5)
...
function buildInterstage() {
  const group = new THREE.Group();
  const cone = lineMesh(new THREE.CylinderGeometry(0.5, 0.52, 0.9, 14));
  cone.position.y = 7.95;
  group.add(cone);
  group.userData.id = 'interstage';
  return group;
}

function buildStage1() {
  const group = new THREE.Group();
  const tank = lineMesh(new THREE.CylinderGeometry(0.52, 0.52, 6.9, 14));
  tank.position.y = 4.0;
  group.add(tank);
  group.userData.id = 'stage1';
  return group;
}
...
function buildFins() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const r = 0.75;
    fin.position.set(Math.cos(angle) * r, 7.15, Math.sin(angle) * r);
    fin.rotation.y = angle;
    fin.rotation.z = Math.PI / 2;
    group.add(fin);
  }
  group.userData.id = 'fins';
  return group;
}
...
function buildLegs() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const r = 0.75;
    leg.position.set(Math.cos(angle) * r, 0.55, Math.sin(angle) * r);
    leg.rotation.z = Math.cos(angle) * 0.55;
    leg.rotation.x = -Math.sin(angle) * 0.55;
    group.add(leg);
  }
  group.userData.id = 'legs';
  return group;
}
...
function buildRocket() {
  ...
  const parts = {};
  [buildFairing(), buildStage2(), buildInterstage(), buildStage1(), buildFins(), buildLegs(), buildEngines()].forEach((g) => {
    rocket.add(g);
...
function showFallback(wrap) {
  const fallback = wrap.querySelector('[data-falcon9-fallback]');
  const canvasHost = wrap.querySelector('[data-falcon9-canvas-host]');
  if (fallback) fallback.style.display = 'block';
  if (canvasHost) canvasHost.style.display = 'none';
}

function init() {
  const wrap = document.getElementById('falcon9-wrap');
  if (!wrap) return;
  const canvas = document.getElementById('falcon9-canvas');
  const canvasHost = wrap.querySelector('[data-falcon9-canvas-host]');
